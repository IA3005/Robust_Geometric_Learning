    ...
    def fit(self, X: ndarray, y=None):
        if self.use_plusplus:
            ...
            for seed in seeds:
                ...
                self.initial_centroids_idx[seed] = initial_idx
                self.initial_centroids_ = X[initial_idx]
...
def kmeans_plusplus_clusters_initialization(X: ndarray, distance: Callable,
    ...
    centroids = []
    indexes = []
    remaining_idx = list(range(0,X.shape[0]))
    ...
    for l in range(1, n_clusters):
        ...
        if n_jobs == 1:
            distances =  [distance(X[indexes],X[j]) for j in remaining_idx]
        else:
            distances = Parallel(n_jobs=n_jobs)(
                    delayed(distance)(X[indexes],X[j]) for j in remaining_idx)
        
        distances = np.asarray(distances).reshape((X.shape[0]-l,l))
        ...
        if verbose > 0:
            pbar.update(1)
            if verbose > 1:
                logging.info(f'Next centroid : {idx}')

    if verbose:
        pbar.close()
    print("chosen centroids for init :",indexes)
    return indexes
